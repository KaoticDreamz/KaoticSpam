...
def validRecipientNum(to_addr,TargetNum):
    valid = False
    while not valid:
        message = False
        if 0 < TargetNum <= 500:
        ...
        if message:
            ...
            while True:
                addr = input(bcolors.OKGREEN + "Enter The Target(s), hit enter to proceed (or \"#\" to use a predefined recipient list): " + bcolors.ENDC)
                if addr == "#":
                    ...
                    save = input (bcolors.OKGREEN + "\nDo you want to save these targets to a new target list? (Y/N): " + bcolors.ENDC)
                    if save.upper() == "Y":
                        nameList = input (bcolors.OKGREEN + "\nName of new list: " + bcolors.ENDC)
                        if nameList in TargetLists.txt:
                            print (bcolors.FAIL + "\nList already exists!\n" + bcolors.ENDC)
                        else:
                            ...
                            if sure.upper() == "Y":
                                TargetLists[nameList] = to_addr
                                saveTargetLists()
                    break
                else:
                    to_addr.append(addr)
    return to_addr,TargetNum

def validSend(send,multiple,TargetNum):
    valid = False
    while not valid:
        message = False
        try:
...
def resetAllLists():
    ...
    try:
        while ("TargetLists.txt"):
            os.remove ("TargetLists.txt")
...
def structure(recipientLists):
    ...
    while True:
        ...
        if addr == "#" and cnt == 1:
            ...
            to_addr.append(addr)
    TargetNum = len (to_addr)
    to_addr,TargetNum = validRecipientNum(to_addr,TargetNum)
    ...
    if limit.lower() == "y":
        send = input(bcolors.FAIL + "Enter the number of emails you want to send: " + bcolors.ENDC)
        send = validSend(send,multiple,TargetNum)
    else:
        send = float ("inf")
    ...
    time.sleep(1)
    return speed,to_addr,body,subject,length,TargetNum,send,bcc
...
def getops(): #Get options and turn off interactive mode
    ...
    parser.add_option('-e', '--num-of-emails', dest='TargetNum', help='the number of email addresses you want to send from')
    ...
    if (ops.interactive == False):
        ...
        if choice == "1":
            ...
            if multiple == "1" or multiple.upper() == "YES":
                ...
                sendSpeed,to_address,body,subject,length,TargetNum,send,bcc = structure(recipientLists)
                if loadingBar and send != float ("inf"):
                    pbar = tqdm(total=(send))
                elif table and TargetNum <= 2:
        ...
        TargetNum = int(ops.TargetNum)  # set this variable to an integer
        if TargetNum <= 500:
            ops.send = ops.send - 2
